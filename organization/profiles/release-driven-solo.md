...
tool-fillgen の current release-driven development で使用する Operating Model を定義します。
...
- host application work state is not mirrored here as a second canonical source
...
- Bun / Biome / TypeScript / Vitest decisions in ADR-0001 remain project-specific toolchain authority.
- source-only validation and host-application integration evidence are separate.
- coverage thresholds are project-local quality evidence and do not substitute for host behavior evidence.
- current release line is `release-x-y-z`; main receives normal integration only from the release PR.
- repository landing uses merge commits only.
